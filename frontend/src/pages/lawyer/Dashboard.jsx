...
export default function LawyerDashboard() {
  ...
  const caseStats = [
    { label: "Active Cases", value: stats?.activeCases || 0, color: "bg-blue-600", lightColor: "bg-blue-100", textColor: "text-blue-600" },
    { label: "Pending", value: stats?.pendingCases || 0, color: "bg-yellow-500", lightColor: "bg-yellow-100", textColor: "text-yellow-600" },
    { label: "Completed", value: stats?.completedCases || 0, color: "bg-green-500", lightColor: "bg-green-100", textColor: "text-green-600" },
    { label: "Urgent", value: stats?.urgentCases || 0, color: "bg-red-500", lightColor: "bg-red-100", textColor: "text-red-600" }
  ];

  const recentActivities = [
    { type: "case", message: "New case assigned: Smith vs. Corporation", time: "2 hours ago", icon: Briefcase, color: "text-purple-600", bg: "bg-purple-100" },
    { type: "document", message: "Document uploaded: Evidence A", time: "5 hours ago", icon: FileText, color: "text-blue-600", bg: "bg-blue-100" },
    { type: "message", message: "New message from client: John Doe", time: "1 day ago", icon: MessageSquare, color: "text-green-600", bg: "bg-green-100" },
    { type: "event", message: "Hearing scheduled for tomorrow", time: "1 day ago", icon: Calendar, color: "text-orange-600", bg: "bg-orange-100" },
  ];
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="inline-flex items-center bg-purple-50 border border-purple-200 px-3 py-1.5 rounded-lg mb-3">
              ...
              <span className="text-xs font-semibold text-purple-600 tracking-wider">LAWYER DASHBOARD</span>
            </div>
            <h1 className="text-xl md:text-2xl lg:text-3xl font-bold text-gray-900">
              Welcome back, <span className="text-blue-600">Adv. {stats?.lawyerName?.split(' ')[0] || "Lawyer"}</span>
            </h1>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 md:p-6 hover:shadow-lg transition-shadow">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4 text-center">
                <p className="text-2xl md:text-3xl font-bold text-blue-600">{stats?.totalClients || 0}</p>
              ...
              <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-4 text-center">
                <p className="text-2xl md:text-3xl font-bold text-green-600">{stats?.successRate || 0}%</p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 md:p-6 hover:shadow-lg transition-shadow">
            ...
            {stats?.recentCases?.length === 0 ? (
              <div className="text-center py-8 bg-gray-50 rounded-lg">
                ...
                <p className="text-xs text-gray-400 mt-1">New cases will appear here</p>
              </div>
            ) : (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 md:p-6 hover:shadow-lg transition-shadow">
            ...
            {stats?.upcomingEvents?.length === 0 ? (
              <div className="text-center py-8 bg-gray-50 rounded-lg">
                ...
                <p className="text-xs text-gray-400 mt-1">Schedule new events to see them here</p>
              </div>
            ) : (
              <div className="space-y-3">
                  ...
                  >
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center text-gray-600">
                        ...
                        <span className="text-xs">Case: {event.caseId?.caseTitle || "N/A"}</span>
                      </div>
                      <div className="flex items-center text-gray-600">
                        <User className="h-3 w-3 mr-2 text-gray-400" />
                        <span className="text-xs">Client: {event.caseId?.clientName || "N/A"}</span>
                      </div>
                      <div className="flex items-center text-gray-600">
                        <Clock className="h-3 w-3 mr-2 text-gray-400" />
                        <span className="text-xs">{event.eventDate ? new Date(event.eventDate).toLocaleString() : "N/A"}</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-3 pb-4 border-b border-gray-100">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Bar ID: {stats?.barId || "N/A"}</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-50 rounded-lg p-3">
                  ...
                  <p className="text-sm font-bold text-gray-900">{stats?.experience || 0} years</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-[10px] text-gray-500 mb-1">Cases Won</p>
                  <p className="text-sm font-bold text-green-600">{stats?.casesWon || 0}</p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center text-sm">
                  ...
                  <span className="text-xs text-gray-600 truncate">{stats?.email || "No email"}</span>
                </div>
                <div className="flex items-center text-sm">
                  <Phone className="h-4 w-4 text-gray-400 mr-3" />
                  <span className="text-xs text-gray-600">{stats?.phone || "No phone"}</span>
                </div>
                <div className="flex items-center text-sm">
                  <MapPin className="h-4 w-4 text-gray-400 mr-3" />
                  <span className="text-xs text-gray-600 truncate">{stats?.location || "No location"}</span>
                </div>
                <div className="flex items-center text-sm">
                  <Award className="h-4 w-4 text-gray-400 mr-3" />
                  <span className="text-xs text-gray-600">Rating: {stats?.rating || "4.8"} ★</span>
                </div>
              </div>

              {/* Specialization Tags */}
              <div className="pt-3 border-t border-gray-100">
                <p className="text-[10px] text-gray-500 mb-2">Specializations</p>
                <div className="flex flex-wrap gap-1.5">
                  {stats?.specializations?.length > 0 ? (
                    stats.specializations.map((spec, index) => (
                      <span key={index} className="px-2 py-1 bg-blue-50 text-blue-700 rounded-lg text-[10px] font-medium border border-blue-100">
                        {spec}
                      </span>
                    ))
                  ) : (
                    <span className="text-xs text-gray-400">No specializations added</span>
                  )}
                </div>
              </div>
            </div>
